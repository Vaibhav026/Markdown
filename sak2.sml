...
(*counts number of hashes in the char list*)
fun count(s,n)=
    if(s=[]) then
    (n,[])    
...
(*s is a converted char list for a particular line*)
...
(*checks whether table has started*)
fun checkstarttable(s)=
    ...
    false  
(*checks whether table has ended*)
fun checkendtable(s)=
    if(s=nil) then
    false
    else if(ord(hd s) = 32 orelse ord(hd s) = 9) then 
...
(*extracts string after hyphen in unordered list*)
fun exul(S)=
    if(S = nil) then
    []
    else if(ord(hd S) = 45 ) then
...
(*extracts string from blockquote *)        
fun extractquote(S)=
    if(S = nil ) then
    []    
...
(*extracts number of blockquotes*)        
fun quo(i) =
    if( i = 0) then
    "\n"
    else
...
fun isul(S) = 
    ...
    isul(tl S)    
    else if( ord(hd S) = 45 andalso tl S <> nil andalso ord(hd(tl S))<>45 ) then
    true
    else 
    false  

(*checks whether string is ordered list*)
fun isstrtlst(s)=
    if(s = nil) then
    (false,[])    
...
fun drawTable(a,bol,strt) =
    ...
    else if( isAlphaNum(hd a)   andalso isspaceline(tl a) ) then
    append("read.html",implode([hd a])^"</TD> </TR>")  
    else
    (
        if( ispace(hd a) andalso isAlphaNum( hd(tl a) ) ) then
        ( let
...
(
    ...
    let
            val m1 = appendLine("read.html","<ul>")
            val e =  exul( explode ( hd(tl L) ) )
            val conv = usefilew(implode(e))
            val m2 = append("read.html","<li>" ^ conv ) 
        in
            fi(tl(tl L),is,quotcount )
        end    
)
else if( isul(explode(hd L)) ) then
(
    let
        val e = exul( explode(hd L) )
        val conv = usefilew(implode(e))
        val m = appendLine("read.html","</li> <li>" ^ conv)
    in
        fi(tl L,is,quotcount)
...
(   
    let
        val m = appendLine("read.html","</li> </ul>")
    in
        fi(tl L,is,quotcount)
    end
